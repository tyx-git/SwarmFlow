 ...
 * 在获取时验证的独立 `.sig` 文件。
 *
 *   node --import tsx scripts/sign-registry.ts <private-key.pem> [registry-dir]
 *
...
if (!keyPath) {
  console.error("usage: node --import tsx scripts/sign-registry.ts <private-key.pem> [registry-dir]");
  process.exit(1);
}
