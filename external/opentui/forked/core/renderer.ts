...
export class CliRenderer extends EventEmitter implements RenderContext {
  ...
  private handleError: (error: Error) => void = ((error: Error) => {
    writeOpenTuiDiag("renderer.error", {
  ...
  constructor(
    ...
    // on every platform — otherwise the OPENTUI_USE_THREAD escape
  ...
  private handleStdinParserFailure(error: unknown): void {
    writeOpenTuiDiag("stdin.parser.failure", {
  ...
  private maybeDumpDiagStateForScrollBurst(mouseEvent: RawMouseEvent): void {
    ...
    writeOpenTuiDiag("renderer.diag_dump", {
      reason: "scroll-burst",
  ...
  private processSingleMouseEvent(mouseEvent: RawMouseEvent): boolean {
    ...
    if (mouseEvent.type === "scroll") {
      ...
      writeOpenTuiDiag("renderer.mouse.scroll", {
    ...
    if (
      ...
      if (canStartSelection && maybeRenderable) {
        const event = this.dispatchMouseEvent(maybeRenderable, mouseEvent)
