...
const ASSISTANT_RENDERER_MODE = getAssistantRenderer();
...
export function OpenTuiApp({
  ...
  const theme = useMemo<DisplayTheme>(() => {
    ...
    // mode (THEME=dark|light or /theme), assume they want the canonical
  ...
  // Commands that run regardless of streaming state.
  const UI_ONLY_COMMANDS = new Set(["/new", "/clear"]);
  ...
  const SESSION_CONFIG_COMMANDS = new Set(["/mcp", "/skills", "/provider", "/model", "/effort", "/permission", "/theme", "/hooks"]);
  ...
  useKeyboard((event) => {
    ...
    // in , so it reaches this handler on every platform.
